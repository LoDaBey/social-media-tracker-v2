...
type Props = {
  personal: SectionProps | null;
  umbrella: SectionProps | null;
};
...
export function FacebookAccountsCard({ personal, umbrella }: Props) {
  if (!personal && !umbrella) return null;
  ...
  return (
    <motion.section
      ...
      aria-label="Facebook accounts card"
      variants={setupCardVariants}
      initial="hidden"
      animate="show"
      exit="exit"
    >
      {personal ? <Section {...personal} /> : null}
      ...
      {personal && umbrella ? (
        <div
          ...
          style={{
            height: 1,
            background: "rgb(from var(--color-coral) r g b / 0.6)",
          }}
          aria-hidden="true"
        />
      ) : null}

      {umbrella ? <Section {...umbrella} /> : null}
    </motion.section>
  );
}
