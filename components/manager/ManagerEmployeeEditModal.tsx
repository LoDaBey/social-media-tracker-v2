...
import { updateManagerEmployeeProfile } from "@/actions/manager-employees";
import type { ManagerEmployeeEditModalProps } from "@/types/manager";
...
export function ManagerEmployeeEditModal({
  open,
  employeeId,
  fullName,
  employeeCode,
  onClose,
}: ManagerEmployeeEditModalProps) {
  return (
    <AnimatePresence>
      {open ? (
        <ManagerEmployeeEditDialog
...
function ManagerEmployeeEditDialog({
  employeeId,
  fullName,
  employeeCode,
  onClose,
}: Omit<ManagerEmployeeEditModalProps, "open">) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await updateManagerEmployeeProfile(employeeId, {
        full_name: trimmedName,
        employee_code: code.trim() === "" ? null : code.trim(),
      });
      if (result.error) {
      ...
      toast.success(`Profile updated for ${trimmedName}.`);
      onClose();
      router.refresh();
    });
  }
  ...
  return (
    ...
    >
      ...
      <motion.div
        role="dialog"
        aria-modal="true"
        aria-labelledby="manager-employee-edit-title"
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h2
              id="manager-employee-edit-title"
        ...
        <div className="mt-6 flex justify-end gap-3">
          <motion.button
            ...
            disabled={pending}
            onClick={onClose}
            aria-label="Cancel employee profile changes"
          ...
          <motion.button
            ...
            onClick={submit}
            aria-label="Save employee profile"
